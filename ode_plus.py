"""
Neural ODE-RNN + Tabular Classifier (ODE Plus)

Pipeline:
  1. Pre-train ODE-RNN encoder end-to-end (BCE loss + validation early-stopping)
...
Usage:
  python ode_plus.py                    # TabPFN
  python ode_plus.py --clf xgboost
  python ode_plus.py --clf catboost
"""
...
from TimeEmbedding import (
    DEVICE, get_all_temporal_features, extract_temporal_data,
    IrregularTimeSeriesDataset, collate_fn, load_and_prepare_patients,
)
from TimeEmbeddingVal import split_patients_train_val
from ode import ODEFunc, rk4_solve
...
# ---------------------------------------------------------------------------
# Hybrid dataset: ODE temporal fields + static features
# ---------------------------------------------------------------------------

class HybridODEDataset(IrregularTimeSeriesDataset):
    """
    Extends IrregularTimeSeriesDataset to also store per-patient static
    feature vectors alongside the temporal data.
    """

    def __init__(self, patients, feature_names, static_encoder,
                 normalization_stats=None):
        # Build temporal data via parent class
        super().__init__(patients, feature_names, normalization_stats)

        # Attach static features — iterate patients in the same order the
        # parent used (those that had non-None temporal data)
        patient_list = patients.patientList if hasattr(patients, "patientList") else patients
        self.static_data = []
        for patient in patient_list:
            from TimeEmbedding import extract_temporal_data as _etd
            times, _, _ = _etd(patient, feature_names)
            if times is None:
                continue
            self.static_data.append(
                torch.tensor(static_encoder.transform(patient), dtype=torch.float32)
            )

    def __getitem__(self, idx):
        temporal, label = super().__getitem__(idx)
        return temporal, label, self.static_data[idx]


def hybrid_ode_collate_fn(batch):
...
# ---------------------------------------------------------------------------
# ODE-RNN Encoder  (same dynamics as ode.py, adds encode() method)
# ---------------------------------------------------------------------------

class ODERNNEncoder(nn.Module):
    """
    ODE-RNN with encode() exposing the hidden state before the MLP head.
    ...
    def encode(self, batch_data: dict) -> torch.Tensor:
        ...
        for t in range(T):
            if t > 0:
                dt = (times[:, t] - times[:, t - 1]).clamp(min=0.0)
                h  = rk4_solve(self.ode_func, h, dt, n_steps=self.ode_steps)
            gru_in = torch.cat([values[:, t], masks[:, t]], dim=-1)
...
def pretrain_encoder(encoder: ODERNNEncoder,
                     train_loader: DataLoader,
                     val_loader: DataLoader,
                     num_epochs: int = 100,
                     eval_every: int = 5,
                     patience: int = 8,
                     lr: float = 5e-4) -> ODERNNEncoder:
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % eval_every == 0:
            ...
            print(f"    [pretrain] epoch {epoch+1:3d} | "
                  ...
                  f"val AUC {val_auc:.4f} | NFE {encoder.ode_func.nfe}")
            if val_auc > best_auc:
                best_auc   = val_auc
...
def extract_combined_features(encoder: ODERNNEncoder,
    ...
    z: ODE-RNN hidden state (encoder.encode()).
    """
    encoder.eval()
    all_X, all_y = [], []
...
def main(clf_type: str = "tabpfn"):
    print("=" * 80)
    print(f"ODE-RNN Plus → {clf_type.upper()} | [Static + Last + ODE encoding]")
    print("=" * 80)
    ...
    for fold, (train_full, test_p) in enumerate(trainTestPatients(patients, seed=SEED)):
        ...
        # Datasets
        train_ds = HybridODEDataset(train_p_obj, temporal_feats, static_enc)
        stats    = train_ds.get_normalization_stats()
        val_ds   = HybridODEDataset(val_p_obj, temporal_feats, static_enc, stats)
        test_ds  = HybridODEDataset(test_p,    temporal_feats, static_enc, stats)
        print(f"  Train {len(train_ds)} | Val {len(val_ds)} | Test {len(test_ds)}")

        train_loader = DataLoader(train_ds, batch_size=32, shuffle=True,
                                  collate_fn=hybrid_ode_collate_fn)
        val_loader   = DataLoader(val_ds,   batch_size=32, shuffle=False,
                                  collate_fn=hybrid_ode_collate_fn)
        test_loader  = DataLoader(test_ds,  batch_size=32, shuffle=False,
                                  collate_fn=hybrid_ode_collate_fn)

        # ---- Step 1: pre-train ODE-RNN encoder ----
        encoder = ODERNNEncoder(input_dim, hidden_dim=64, ode_steps=4).to(DEVICE)
        print("  Pre-training ODE-RNN encoder...")
    ...
    ax.set_title(f"ODE-RNN Plus ({clf_type.upper()}): AKD Prediction",
                 fontsize=14, fontweight="bold")
    ax.legend(loc="lower right"); ax.grid(alpha=0.3)
    plt.tight_layout()
    os.makedirs("result", exist_ok=True)
    out_path = f"result/ode_plus_{clf_type}.png"
    plt.savefig(out_path, dpi=300)
    print(f"\nROC plot saved to {out_path}")

    # Summary
    print("\n" + "=" * 80)
    print(f"FINAL RESULTS — ODE-RNN Plus ({clf_type.upper()})")
    print("=" * 80)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="ODE-RNN Plus: temporal encoder + tabular classifier")
